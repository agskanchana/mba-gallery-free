...
function medbeafgallery_get_case_limit_notice() {
    ...
    if ($total_cases >= $max_cases) {
        return sprintf(
            ...
            __('You have reached the maximum limit of %d cases. Upgrade to add more cases.', 'medical-before-after-gallery'),
            $max_cases
        );
    }
...
function medbeafgallery_get_categories_with_images() {
    // Get the terms with count information
    $categories = get_terms(array(
        'taxonomy' => 'medbeafgallery_category', // make sure this is your actual taxonomy
        'hide_empty' => false, // we'll filter empty ones in the render function
    ));
